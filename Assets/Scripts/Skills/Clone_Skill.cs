...
public class Clone_Skill : Skill
{
    ...
    public void CreateClone(Transform _cloneTransform, Vector3 _offset)
	{
		...
		newClone.GetComponent<Clone_Skill_Controller>().
			SetupClone(_cloneTransform, cloneDuration, canAttack, _offset, FindClosestEnemy(newClone.transform), canDuplicateClone, changeToDuplicate,player);
	}
	...
	public void CreateCloneOnCounterAttack(Transform _enemyTranform)
	{
		if (canCreateCloneOnCounterAttack)
			StartCoroutine(CreateCloneWithDelay(_enemyTranform, new Vector3(2 * player.facingDir, 0)));
	}
